...
def main(args, cfg_env=None):
    ...
    for epoch in range(epochs):
        ...
        (delta_lyapunov_critic_history[current_delta_lyapunov_critic_index]).load_state_dict(delta_lyapunov_critic.state_dict())
        current_delta_lyapunov_critic_index=(current_delta_lyapunov_critic_index+1) % 10
        ...
        for _ in range(config["learning_iters"]):
            for (
                ...
                if terminate_b.any():
                    initial_target_value_c_mean += (target_value_c_b[terminate_b]).mean()
                    start_states+=1
                ...
                total_lyapunov_loss = 10*((torch.max(lagrange_coefficient,torch.tensor(0., dtype=lagrange_coefficient.dtype, device=lagrange_coefficient.device)))*lyapunov_loss).mean()
                #printing stuff
                ...
                if epoch>15:
                    total_loss = loss_pi + 2*loss_r + loss_c + total_lyapunov_loss + delta_lyapunov_critic_error_mean + mean_initial_lyapunov_penalty \
                        if config.get("use_value_coefficient", False) \
                        else loss_pi + loss_r + loss_c + total_lyapunov_loss + mean_initial_lyapunov_penalty + delta_lyapunov_critic_error_mean 
                else:
                    total_loss = loss_pi + 2*loss_r + loss_c + total_lyapunov_loss + mean_initial_lyapunov_penalty + delta_lyapunov_critic_error_mean \
                        if config.get("use_value_coefficient", False) \
                        else loss_pi + loss_r + loss_c + total_lyapunov_loss + mean_initial_lyapunov_penalty + delta_lyapunov_critic_error_mean 
