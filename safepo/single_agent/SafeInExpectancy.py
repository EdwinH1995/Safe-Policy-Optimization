...
def main(args, cfg_env=None):
    ...
    # create Lyapunov Function and Critic Expectancy
    safety_network = LyapunovFunction(
        obs_dim=obs_space.shape[0],  # Use the observation space dimension
        hidden_sizes=config["hidden_sizes"]  # Use the hidden sizes from config
    ).to(device)
    safety_network_optimizer = torch.optim.Adam(
        safety_network.parameters(), lr=3e-4
    )
    ...
    # Initialize terminate and is_start

    terminate=torch.zeros(obs_space.shape[0], dtype=torch.bool,device=device)
    is_start=torch.ones(obs_space.shape[0], dtype=torch.bool,device=device)
    ...
    for epoch in range(epochs):
        ...
        for steps in range(local_steps_per_epoch):
            ...
            with torch.no_grad():
                ...
                if lyapunov_next.dim() == 0:  # If 'terminate' is a scalar
                    lyapunov_next = lyapunov_next.unsqueeze(0)  # Adds a dimension """
        ...
        update_counts = 0
        expectancy_goal_old=expectancy_goal_new
        expectancy_goal_new=0.
        lagrange_coeff = max(lagrange_coeff+expectancy_goal_old,0.)
        ...
        for _ in range(config["learning_iters"]):
            for (
                ...
                safety_network_optimizer.zero_grad()
                gamma=config["gamma"]
                ...
                lyapunov_loss=(ratio_cliped[non_terminating])*(cost_b[non_terminating]+gamma*torch.relu(lyapunov_next)-lyapunov_current)
                with torch.no_grad():
                    expectancy_goal_new += lyapunov_loss.mean()
                lyapunov_loss_mean=lyapunov_loss.mean()
                total_loss = loss_pi + 2*loss_r + loss_c + (max(lagrange_coeff,0.)+max(expectancy_goal_new,0.))*lyapunov_loss_mean\
