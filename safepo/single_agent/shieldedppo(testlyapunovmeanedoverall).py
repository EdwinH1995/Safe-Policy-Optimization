...
def main(args, cfg_env=None):
    ...
    for epoch in range(epochs):
        ...
        for steps in range(local_steps_per_epoch):
            ...
            # print("Terminated type:", terminated.dtype)
            # print("Terminate type:", terminate.dtype)
            # print(f"obs shape: {obs.shape}, act shape: {act.shape}, reward shape: {reward.shape}, lyapunov shape: {lyapunov_current.shape},terminate shape: {terminate.shape}")
            buffer.store(
                obs=obs,
                act=act,
                reward=reward,
        ...
        for _ in range(config["learning_iters"]):
            for (
                ...
                if active_mask.any():
                    ...
                    ratio_next = lyapunov_next / lyapunov_next_b
                    ...
                    penalty_current = torch.relu(torch.abs(clipped_ratio_current - 1) - epsilon) * torch.abs(lyapunov_current_b)
                    penalty_next = torch.relu(torch.abs(clipped_ratio_next - 1) - epsilon) * torch.abs(lyapunov_next_b)
                    ...
                    total_lyapunov_loss = lambda_penalty * torch.relu(lyapunov_loss)
                ...
                mean_initial_lyapunov_penalty=initial_lyapunov_penalty.mean()
                ...
                total_loss = loss_pi + 2*loss_r + loss_c + total_lyapunov_loss + mean_initial_lyapunov_penalty\
                    if config.get("use_value_coefficient", False) \
                    else 10*loss_pi + loss_r + loss_c + total_lyapunov_loss + mean_initial_lyapunov_penalty
                """ print("Before backward: ", total_loss.requires_grad)  # Should be True
